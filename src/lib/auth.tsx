...
/**
 * Regex for stripping email domains.
 * Exported for shared use across auth profile creation and UI name extraction.
 */
export const EMAIL_CLEAN_REGEX = /@.*/;

/**
 * Regex for extracting initials from a name, preserving word boundaries.
 * Optimized for O(1) matching in loop.
 */
export const INITIALS_REGEX = /(?:^|[._\-\s])([\p{L}\p{N}_])/gu;

/** Regex for stripping leading decorative characters from a name. */
export const INITIAL_CLEAN_REGEX = /^[._\-\s]+/;

/** Regex for stripping non-alphanumeric characters for a conservative fallback. */
