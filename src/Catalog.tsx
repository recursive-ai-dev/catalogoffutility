...
interface EntryGridProps {
  entries: typeof SEARCHABLE_ENTRIES;
  onCardSelect: (entry: AppEntry) => void;
  onTagSelect: (tag: string) => void;
  onResetFilters: () => void;
...
const SystemLogs = React.memo(function SystemLogs({ mountTime }: SystemLogsProps) {
  const { user } = useAuth();
  return (
...
export const Catalog = React.memo(function Catalog({
  ...
  const searchActive = searchQuery !== "" || selectedTag !== DEFAULT_TAG;
    ...
    <div className="relative flex h-screen w-full flex-col md:flex-row overflow-hidden bg-black font-sans text-white antialiased">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative z-10">
        ...
        >
          ...
          <SystemLogs mountTime={mountTime} />
        </main>
      </div>
    </div>
  );
