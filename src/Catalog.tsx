...
const Sidebar = React.memo(function Sidebar({
  ...
  const handleForgetClick = () => {
    if (!isConfirmingForget) {
      ...
      confirmTimerRef.current = setTimeout(() => setIsConfirmingForget(false), 3000);
    } else {
    ...
    <div className="w-full md:w-72 shrink-0 flex flex-col border-b md:border-b-0 md:border-r border-white/10 bg-black/40 backdrop-blur-xl z-20">
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-2">
        ...
        <button
          ...
          aria-label={isConfirmingForget ? "Confirm purging memories" : "Forget (Reset filters)"}
        >
          <span
            className={`material-symbols-outlined text-xl font-light transition-colors ${isConfirmingForget ? "text-red-400" : "text-white"}`}
            aria-hidden="true"
          >
            {isConfirmingForget ? "priority_high" : "delete"}
          </span>
          <span
            className={`font-light uppercase tracking-widest text-xs transition-colors ${isConfirmingForget ? "text-red-400" : "text-white"}`}
          >
            {isConfirmingForget ? "Are you sure?" : "Forget"}
          </span>
        </button>
        <button
          className="group flex items-center gap-4 px-4 py-3 rounded-lg border border-transparent hover:bg-white/5 transition-all duration-300 cursor-pointer w-full text-left focus-visible:ring-1 focus-visible:ring-white/30 outline-none"
