...
/**
 * Optimized regex for extracting initials while preserving underscore/dot/dash boundaries.
 * Pre-allocated at the module level to avoid repeated instantiation.
 */
...
const UserSection = React.memo(function UserSection() {
  ...
  // Memoize derived profile data to prevent redundant string operations and
  // formatting during re-renders (e.g. when last_seen_at updates).
  const { displayName, initialsLabel, joined } = useMemo(() => {
    const dName = profile?.username ?? user.email?.split("@")[0] ?? "entity";
    const jDate = profile?.created_at
      ? (() => {
...
export const Catalog = React.memo(function Catalog({
  ...
  // Chain 1 (BrowseFilter): normalize the query before deferring it.
  // This ensures useDeferredValue (and the subsequent O(N) filter) only
  // triggers when the functional query actually changes, ignoring extra whitespace.
