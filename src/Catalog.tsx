    ...
    <div className="w-full md:w-72 shrink-0 flex flex-col border-b md:border-b-0 md:border-r border-white/10 bg-black/40 backdrop-blur-xl z-20">
      ...
      <nav className="flex-1 overflow-y-auto py-6 px-4 flex flex-col gap-2">
        ...
        <button
          ...
          aria-label={isConfirming ? "Confirm purging memories" : "Forget filters"}
        >
          <span
            className={`material-symbols-outlined text-xl font-light ${isConfirming ? "text-red-400" : "text-white"}`}
            aria-hidden="true"
          >
            {isConfirming ? "priority_high" : "delete"}
          </span>
          <span
            className={`font-light uppercase tracking-widest text-xs ${isConfirming ? "text-red-400" : "text-white"}`}
          >
