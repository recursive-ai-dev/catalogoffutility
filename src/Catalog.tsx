...
/** O(1) random selection pool for anonymous users — excludes missing and auth-gated entries. */
const NAVIGABLE_ANON = SEARCHABLE_ENTRIES.filter(
  (e) => !e.missing && !e.requiresAuth,
);

/** O(1) random selection pool for authenticated users — excludes only missing entries. */
...
export const Catalog = React.memo(function Catalog({
  ...
  const handleCardSelect = useCallback(
    ...
    [isLoggedIn, onShowAuthModal, onSelectApp],
  );
  ...
  }, [isLoggedIn, handleCardSelect, showNotification]);
