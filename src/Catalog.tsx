...
// Module-level regex for initials extraction to avoid re-compilation on every call.
...
const UserSection = React.memo(function UserSection() {
  ...
  // Memoize initials extraction to avoid re-calculation on every render.
  const initialsLabel = useMemo(() => initials(displayName), [displayName]);

  // Memoize date formatting to avoid repeated Date object creation and formatting.
...
export const Catalog = React.memo(function Catalog({
  ...
  // Normalize the query first to avoid redundant filtering on non-functional changes (e.g. trailing spaces).
  const normalizedQuery = useMemo(() => searchQuery.trim().toLowerCase(), [searchQuery]);
  const deferredSearchQuery = React.useDeferredValue(normalizedQuery);
