...
/**
 * Appends a new entry to the log list, ensuring it doesn't exceed MAX_LOGS.
 * Automatically assigns a stable incrementing ID for efficient React reconciliation.
 *
 * Performance: Using stable IDs instead of array indices as keys prevents
 * re-rendering the entire list when an entry is evicted from the top.
 */
function appendLog(
  prev: LogEntry[],
...
export const Chamber = React.memo(function Chamber({ app, onBack, initialError, clock }: ChamberProps) {
  ...
  return (
    <div className="font-sans bg-black text-white antialiased overflow-hidden h-screen flex flex-col selection:bg-white/20 selection:text-white relative">
      ...
      {/* Main Content Area */}
      <main id="main-content" tabIndex={-1} className="flex-1 flex overflow-hidden relative z-20 outline-none">
        {/* Game Viewport Area */}
