...
function isSafeImageSrc(src: string): boolean {
  ...
  // We only short-circuit https: if it contains no '@' character (no credentials).
  // http: must always proceed to hostname validation (BUG-06c).
  if (src.startsWith("https://") && !src.includes("@")) return true;
  if (src.startsWith("data:")) return SAFE_DATA_URL_REGEX.test(src);

  try {
    const url = new URL(src);
    // Reject URLs with embedded credentials to prevent SSRF bypass or leakage.
