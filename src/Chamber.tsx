...
function isSafeImageSrc(src: string): boolean {
  ...
  // Short-circuit data: URLs as they are not parsed by the URL constructor in the same way.
  if (src.startsWith("data:")) return SAFE_DATA_URL_REGEX.test(src);

  try {
    const url = new URL(src);
    // Reject URLs with embedded credentials to prevent SSRF-like probes (BUG-06d).
