...
function isSafeImageSrc(src: string): boolean {
  ...
  // Short-circuit data: protocol as it does not have a hostname to validate.
  if (src.startsWith("data:")) return SAFE_DATA_URL_REGEX.test(src);

  try {
    const url = new URL(src);

    // Block URLs with embedded credentials to prevent credential leaking
    // and as a defense-in-depth measure against SSRF/CSRF (BUG-SEC-01).
