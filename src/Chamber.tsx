...
function isSafeImageSrc(src: string): boolean {
  ...
  // Short-circuit data: URLs to avoid unnecessary URL parsing if the regex matches.
  if (src.startsWith("data:")) return SAFE_DATA_URL_REGEX.test(src);

  try {
    const url = new URL(src);

    // Reject URLs with embedded credentials to prevent credential leakage or
    // potential SSRF bypasses via authentication components.
