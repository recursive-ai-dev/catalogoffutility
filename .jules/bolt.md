...
## 2026-04-20 - Search Normalization Memoization & Navigation Guards

**Learning:** When using O(N) filtering on every keystroke, normalizing the search query (trim/lowercase) inside the filter's `useMemo` triggers redundant calculations for non-functional input (e.g., trailing spaces). Extracting normalization into its own `useMemo` and depending on that primitive significantly reduces main-thread work. Additionally, guard clauses in navigation handlers prevent unnecessary browser work like triggering View Transitions when no state change would occur.
**Action:** Always extract input normalization into standalone memoized primitives. Add short-circuits to navigation callbacks to avoid firing expensive UI transitions for redundant selection events.
