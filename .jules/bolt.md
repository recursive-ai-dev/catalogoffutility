...
## 2026-05-10 - Double-Memoization for Deferred Search

**Learning:** When using `useDeferredValue` for search inputs, normalizing the raw query (trim, lowercase) in a dedicated `useMemo` before it reaches the deferred hook or the filter logic prevents redundant O(N) filter operations when the user types non-functional characters like trailing spaces.
**Action:** Implement "double-memoization" for all search inputs: normalize the raw state in `useMemo` before use in expensive filtering or deferral chains.
